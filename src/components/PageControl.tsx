...
export interface PageControlProps {
  setColorOnIndex?: (index: number) => {borderColor: any; backgroundColor: any};
  /**
...
class PageControl extends PureComponent<PageControlProps, State> {
  ...
  static getDerivedStateFromProps(
    ...
    if (currentPage !== prevPage) {
      ...
      if (currentPage >= prevLargeIndicatorsOffset + NUM_LARGE_INDICATORS) {
        ...
        newState.pagesOffset = Math.max(
          0,
          currentPage - NUM_LARGE_INDICATORS - 1,
        );
  ...
  getSize(index: number): undefined | number {
    ...
    if (index < 0 || index > numOfPages - 1) {
      ...
      index >= largeIndicatorsOffset &&
      index < largeIndicatorsOffset + NUM_LARGE_INDICATORS
    ) {
      return size;
    } else if (
      index === largeIndicatorsOffset - 1 ||
      index === largeIndicatorsOffset + NUM_LARGE_INDICATORS
    ) {
      return mediumSize;
    } else if (
      index === largeIndicatorsOffset - 2 ||
      index === largeIndicatorsOffset + NUM_LARGE_INDICATORS + 1
    ) {
      return smallSize;
    }
  }
  ...
  renderSameSizeIndicators() {
    ...
    const size = Array.isArray(sizeFromProps)
      ? sizeFromProps[2]
      : sizeFromProps;

    return _.map(_.times(numOfPages), (index) =>
      this.renderIndicator(index, size, enlargeActive),
    );
  }
